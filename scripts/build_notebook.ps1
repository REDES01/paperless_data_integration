...
# Run from the integration repo root: .\scripts\build_notebook.ps1

$ErrorActionPreference = 'Stop'
$outPath = Join-Path $PSScriptRoot '..\provision_chameleon.ipynb' | Resolve-Path -ErrorAction SilentlyContinue
if (-not $outPath) {
...
4. Clones the three sibling repos (``paperless_data``, ``paperless-ngx-fork``, ``paperless_data_integration``) into ``~/``
...
9. Brings up the Paperless stack with the network override
10. Verifies cross-stack DNS works
11. Prints access URLs for every UI
...
- ``~/paperless-ngx-fork/`` — Paperless UI fork
- ``~/paperless_data_integration/`` — this repo
"@

    New-CodeCell @"
DATA_REPO        = "https://github.com/REDES01/paperless_data.git"
FORK_REPO        = "https://github.com/REDES01/paperless-ngx-fork.git"
...
Source builds of Paperless-ngx require an explicit secret key. We generate one on the VM and write it into ``paperless/docker-compose.env``.
"@
...
echo 'Secret key written to docker-compose.env'
''')
"@
...
"@
    ...
    New-MdCell "## Step 12 — Bring up the Paperless stack"
...
"@
    ...
    New-MdCell @"
## Step 13 — Verify cross-stack DNS

This runs ``getent hosts`` from inside the Paperless webserver container against ``postgres``, ``minio``, ``redpanda``, and ``qdrant``. All four should resolve to private IPs on ``paperless_ml_net``.
"@
...
"@
    ...
    New-MdCell "## Step 14 — Print access URLs"
...
# ConvertTo-Json escapes forward slashes in strings (nonstandard for JSON but common).
# Jupyter tolerates it, but strip it for cleanliness.
$json = $json -replace '\\/', '/'
